...
public class Main {
	 ...
	 */
		...
		public static void medidaEstadistica (int [] HammingR , DefaultCategoryDataset Datos,DefaultCategoryDataset Datos2) {
			...
			// Dibujar graficas
			Grafica = ChartFactory.createBarChart("SHA-1", "Pruebas", "Distancias de Hamming", Datos, PlotOrientation.VERTICAL, true, true, false);
			Grafica2 = ChartFactory.createBarChart("SHA-1", "Pruebas", "Probabilidades de distribucion", Datos2, PlotOrientation.VERTICAL, true, true, false);
			
			ChartPanel Panel = new ChartPanel(Grafica);
			ChartPanel Panel2 = new ChartPanel(Grafica2);
			
			JFrame Ventana = new JFrame("Estudio estadistico SHA-1");
			Ventana.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
			Ventana.getContentPane().add(Panel, BorderLayout.CENTER);
			Ventana.pack();
			Ventana.setVisible(true);
			
			JFrame Ventana2 = new JFrame("Estudio estadistico SHA-1");
	...
	public static void main (String[] args) throws NoSuchAlgorithmException
	{
		...
		int[] HammingR = new int[160];	
		// array para las pruebas
		int hPunt = 0;
		// var para los valores haming 
		float probabilidad[] = new float[160];
		// array para la distribucion de probabilidades 
		byte [] entrada = new byte [20] ;
		byte [] salida = new byte [20];
		byte [] salidaH = new byte [20];
		for(int i=0; i<20; i++)
			entrada[i] = 0;
		salida = pan.digest(entrada);
		//este for me dice el numero de bytes ojo bytes osea si yo tengo 160 bit tengo 160/8 = 20bytes
		for(int i=0; i<20; i++){
			//no tocar pasa de bytes a bit
			for(int j=0; j<8; j++){				
				...
				Datos.addValue(HammingR[hPunt], "Salidas de la funcion SHA-1", Integer.toString(hPunt+1));
